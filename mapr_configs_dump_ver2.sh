...
if [ ! -d $BACKUP_DIR ];then
  echo "#Creating Backup Directory : $BACKUP_DIR" >> $logfile
  mkdir -p $BACKUP_DIR
fi
...
HTTPS_CONF=$(find /opt/mapr/httpfs/httpfs-1.0/ -type f -name '*.xml' 2>/dev/null | cut -d'/' -f4-)
ZOOKEEPER_DATA=zkdata

#EchoSystem Components
HIVE=$(find /opt/mapr -maxdepth 2 -type d -name 'hive-[0-9].[0-9]*' -exec ls -dv '{}' ';' 2>/dev/null | sort --version-sort | tail -n 1 | cut -d'/' -f4-)
DRILL=$(find /opt/mapr -maxdepth 2 -type d -name 'drill-[0-9].[0-9]*' -exec ls -dv '{}' ';' 2>/dev/null | sort --version-sort | tail -n 1 | cut -d'/' -f4-)
...
HUE=$(find /opt/mapr -maxdepth 2 -type d -name 'hue-[0-9].[0-9]*' -exec ls -dv '{}' ';' 2>/dev/null | sort --version-sort | tail -n 1 | cut -d'/' -f4-)
#FLUME=$(find /opt/mapr -maxdepth 2 -type d -name 'flume-[0-9].[0-9]*' -exec ls -dv '{}' ';' 2>/dev/null | sort --version-sort | tail -n 1 | cut -d'/' -f4)
#PIG=$(find /opt/mapr -maxdepth 2 -type d -name 'pig-[0-9].[0-9]*' -exec ls -dv '{}' ';' 2>/dev/null | sort --version-sort | tail -n 1 | cut -d'/' -f4)
#IMPALA=$(find /opt/mapr -maxdepth 2 -type d -name 'impala-[0-9].[0-9]*' -exec ls -dv '{}' ';' 2>/dev/null | sort --version-sort | tail -n 1 | cut -d'/' -f4)
#OOZIE=$(find /opt/mapr -maxdepth 2 -type d -name 'oozie-[0-9].[0-9]*' -exec ls -dv '{}' ';' 2>/dev/null | sort --version-sort | tail -n 1 | cut -d'/' -f4)

echo -e "#Configurations Backup Started ---> $BACKUP_DIR\n" >> $logfile
for host in $IP_ADD
do
LOCAL_BKP="/mapr/${clustername}/managed_services/mapr_conf_bkps/$DATE1/mapr_backup_configs_${host}.tgz"
